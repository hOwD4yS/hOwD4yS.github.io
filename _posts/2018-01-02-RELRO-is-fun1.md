...
이제 RELRO 에서 보호기법을 이용한 exploit 종류 밑 , 라이브러리 로딩방식을 모두 살펴보도록 하죠!
...
위에 빨간색으로 네모난 부분이 범위안에 들어가는 주소들이는 맨 오른쪽이 이름입니다.

TIp . WA이라고 되어있는건 쓰고 읽을 수 있다는 권한입니다. 하지만 대부분이 RELRO를 보호기법이 적용되지 않은 상태입니다.

RELRO는 바이너리가 실행되기전 _start라는 함수를 거쳐서 실행 되는데 그때 mprotect를 통해 결정이 됩니다.

NO RELRO는 저 모든 부분을 Write & Read 할 수 있는 권한이 있습니다,, 매우 취약합니다.

이를 이용한 exploit 기법은 밑에서 살펴 보도록 하죠
